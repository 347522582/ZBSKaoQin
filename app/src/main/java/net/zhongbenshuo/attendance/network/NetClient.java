...
import net.zhongbenshuo.attendance.ZBSApplication;
import net.zhongbenshuo.attendance.constant.ApkInfo;
import net.zhongbenshuo.attendance.constant.NetWork;
import net.zhongbenshuo.attendance.contentprovider.SPHelper;
import net.zhongbenshuo.attendance.network.retrofit.ProgressListener;
import net.zhongbenshuo.attendance.network.retrofit.ProgressResponseBody;
...
public class NetClient {
    ...
    private NetClient(String baseUrl) {
        ...
        //设置缓存目录
        File cacheFile = new File(ZBSApplication.getInstance().getExternalCacheDir(), CACHE_NAME);
        //生成缓存，50M
        Cache cache = new Cache(cacheFile, 1024 * 1024 * 50);
        //缓存拦截器
        Interceptor cacheInterceptor = (chain) -> {
            Request request = chain.request();
            if (NetworkUtil.isNetworkAvailable(ZBSApplication.getInstance())) {
                //网络可用,强制从网络获取数据
